...
void
dios_ssp_vad_mix_process_subfunc(VAD_COUNTER *srv, float signaldev, float history_certainty, float certainty_level) {
...
int dios_ssp_vad_mix_process(int apm_flag, int dt_flag, void *vad_counter_handle, float signaldev, int *vad_flag,
  ...
  *vad_flag = 0;
  if (state == 1) {
    ...
    srv->vad_certainty_cnt_thld = ((srv->vad_certainty_cnt_thld < CERTAINTY_SUM_THRESH_LOW) ? CERTAINTY_SUM_THRESH_LOW
                                                                                            : srv->vad_certainty_cnt_thld);
  } else {
    srv->vad_certainty_cnt_thld = CERTAINTY_SUM_THRESH;
  }
