...
int dios_ssp_energy_vad_process(void *energyvad_ptr, float *inbuf) {
  ...
  for (i = vad_param->lo_chan; i <= vad_param->hi_chan; i++) {
    ...
    for (j = j1; j <= j2; j++) {
      vad_param->energy += vad_param->vad_fftbuf_out[j].r * vad_param->vad_fftbuf_out[j].r +
                           vad_param->vad_fftbuf_out[j].i * vad_param->vad_fftbuf_out[j].i;
    }
  ...
  if ((vad_param->update_flag == TRUE) || (vad_param->energyvad_noise_est->noise_change_flag == 1 &&
                                           vad_param->energyvad_noise_est->noise_change_update_flag)) {
    for (i = vad_param->lo_chan; i <= vad_param->hi_chan; i++) {
      vad_param->ch_noise[i] =
              vad_param->cne_sm_fac * vad_param->ch_noise[i] + (1 - vad_param->cne_sm_fac) * vad_param->ch_energy[i];
